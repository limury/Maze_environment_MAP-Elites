...
for obj in values:
    ...
    for pre in values[obj]:
        if ('gaussian-AMB-end_pos' in pre or 'refactored' in pre):#"FalseReplace" in pre):# or "batch" not in pre):# "RMB" not in pre and "gaussian" not in pre and "random" not in pre and "SSP" not in pre and (values["Magnet"][pre][0][-1] >= 500 or values["Magnetic Stick"][pre][0][-1] >= 3200 or values["Velcro"][pre][0][-1] >= 400 or values["Velcro Stick"][pre][0][-1] >= 3200)):# and ("random" not in pre) and ("gaussian" not in pre)):
